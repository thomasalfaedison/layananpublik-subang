<?php

namespace App\Constants;

class LayananConstant
{
    public const RouteIndex = 'layanan.index';
    public const RouteCreate = 'layanan.create';
    public const RouteCreateProcess = 'layanan.createProcess';
    public const RouteUpdate = 'layanan.update';
    public const RouteUpdateProcess = 'layanan.updateProcess';
    public const RouteView = 'layanan.view';
    public const RouteDelete = 'layanan.delete';
    public const RouteExportPdf = 'layanan.export-pdf';
    public const RouteExportExcelAll = 'layanan.export-excel-all';
    public const RouteExportPdfAll = 'layanan.export-pdf-all';
}
